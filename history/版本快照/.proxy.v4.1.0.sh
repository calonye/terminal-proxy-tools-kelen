...
# ==================== 全功能终端代理配置 (v4.1.0 by Kelen+Gemini) - 开始 ====================
#
# @description: 一个强大、安全且注释详尽的终端代理管理脚本。
#               v4.1.0 在 v4.0.0 的基础上继续加固：补充依赖探测、分层状态输出和 Git/SSH 写入失败提示。
# @author:      Kelen+Gemini
# @version:     4.1.0 (稳健性增强版)
#
# --- 使用方法 ---
#
# 请选择以下任意一种方式进行安装：
...
function _proxy_status_flag() {
    if [ "$1" -eq 0 ]; then
        printf '\033[32m可用\033[0m'
    else
        printf '\033[31m缺失\033[0m'
    fi
}
...
function _proxy_ssh_on() {
    ...
    if ! _proxy_has_command nc; then
        echo -e "  \033[33m[!]\033[0m 未检测到 \`nc\`，已跳过 SSH 代理配置。请先安装支持 ProxyCommand 的 netcat。"
        return 1
    fi

    ssh_bypass_host_line="$(_proxy_render_ssh_host_line)"

    if ! mkdir -p -m 700 ~/.ssh; then
        echo -e "  \033[31m[x]\033[0m 无法创建 \`~/.ssh\` 目录，已跳过 SSH 代理配置。"
        return 1
    fi
    ...
    if ! grep -q "$SSH_PROXY_MARKER_START" ~/.ssh/config 2>/dev/null; then
        ...
        } >> ~/.ssh/config && chmod 600 ~/.ssh/config; then
            echo -e "  \033[32m[✓]\033[0m 已自动配置 SSH 代理，并为本地局域网目标启用直连规则。"
        else
            echo -e "  \033[31m[x]\033[0m 写入 \`~/.ssh/config\` 失败，请检查权限。"
            return 1
        fi
    else
...
function _proxy_ssh_off() {
    if [ -f ~/.ssh/config ] && grep -q "$SSH_PROXY_MARKER_START" ~/.ssh/config; then
        ...
        if sed -i.bak "/${SSH_PROXY_MARKER_START}/,/${SSH_PROXY_MARKER_END}/d" ~/.ssh/config; then
            echo -e "  \033[33m[✓]\033[0m 已自动移除 SSH 代理配置。您的原始配置已备份至: ~/.ssh/config.bak"
        else
            echo -e "  \033[31m[x]\033[0m 从 \`~/.ssh/config\` 移除代理配置失败，请手动检查。"
            return 1
        fi
    fi
}
...
function proxy_on() {
    ...
    if _proxy_has_command git; then
        if git config --global http.proxy "http://${PROXY_HOST}:${PROXY_HTTP_PORT}" \
            && git config --global https.proxy "http://${PROXY_HOST}:${PROXY_HTTP_PORT}"; then
            echo -e "  \033[32m[✓]\033[0m 已配置 Git 代理 (影响所有 https:// 协议的 Git 操作)"
        else
            echo -e "  \033[31m[x]\033[0m Git 代理写入失败，请检查全局 Git 配置权限。"
        fi
    else
        echo -e "  \033[33m[!]\033[0m 未检测到 \`git\`，已跳过 Git 代理配置。"
    fi
...
function proxy_off() {
    ...
    if _proxy_has_command git; then
        ...
        echo -e "  \033[33m[!]\033[0m 未检测到 \`git\`，已跳过 Git 代理清理。"
    fi
    _proxy_ssh_off
...
function proxy_status() {
    ...
    local git_proxy
    local ssh_bypass_host_line
    local git_dep_status
    local nc_dep_status

    effective_no_proxy="$(_proxy_join_csv $(_proxy_no_proxy_entries))"
    ssh_bypass_host_line="$(_proxy_render_ssh_host_line)"
    _proxy_has_command git
    git_dep_status=$?
    _proxy_has_command nc
    nc_dep_status=$?

    echo -e "\033[1m--- 当前代理状态 ---\033[0m"
    echo -e "依赖检查: git=$(_proxy_status_flag "$git_dep_status") | nc=$(_proxy_status_flag "$nc_dep_status")"
    if [ -n "$http_proxy" ] || [ -n "$HTTP_PROXY" ]; then
        echo -e "环境变量: \033[32m开启\033[0m"
    ...
    echo "  NO_PROXY: ${effective_no_proxy}"
    if [ "$git_dep_status" -eq 0 ]; then
        git_proxy=$(git config --global --get http.proxy 2>/dev/null)
    else
        git_proxy=""
    fi
    if [ -n "$git_proxy" ]; then
        echo -e "Git 代理: \033[32m开启\033[0m"
        echo "  git config --global http.proxy: $git_proxy"
    elif [ "$git_dep_status" -eq 0 ]; then
        echo -e "Git 代理: \033[33m关闭\033[0m"
    else
        echo -e "Git 代理: \033[31m不可用\033[0m (\`git\` 未安装)"
    fi
    if grep -q "$SSH_PROXY_MARKER_START" ~/.ssh/config 2>/dev/null; then
         echo -e "SSH 代理: \033[32m开启\033[0m (由本脚本自动管理)"
         echo "  SSH 局域网直连: ${ssh_bypass_host_line}"
    else
         echo -e "SSH 代理: \033[33m关闭\033[0m (或由用户手动管理)"
         echo "  SSH 局域网直连规划值: ${ssh_bypass_host_line}"
    fi
    echo "--------------------"
}

# ===================== 全功能终端代理配置 (v4.1.0 by Kelen+Gemini) - 结束 =====================
